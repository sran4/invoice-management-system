...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Immediate data fetching without debounce for better UX
  ...
  const stats = {
    ...
    pendingInvoices: yearToDateInvoices.filter((i) => i.status === "sent")
      .length,
    pendingAmount: yearToDateInvoices
      .filter((i) => i.status === "sent")
      .reduce((sum, i) => sum + i.total, 0),
    paidInvoices: yearToDateInvoices.filter((i) => i.status === "paid").length,
    draftInvoices: yearToDateInvoices.filter((i) => i.status === "draft")
      .length,
    draftAmount: yearToDateInvoices
      .filter((i) => i.status === "draft")
      .reduce((sum, i) => sum + i.total, 0),
    overdueInvoices: yearToDateInvoices.filter((i) => i.status === "overdue")
      .length,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
            📊 Dashboard shows year-to-date data (Jan 1, {currentYear} -
            present)
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
          <AnimatedCard
            delay={0.1}
            className="gradient-card cursor-pointer group motion-safe"
          >
          ...
          >
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Badge className="gradient-green-emerald text-white">
                  Analytics
                </Badge>
              </div>
            </CardHeader>
          ...
          >
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <motion.div
                  className="p-3 bg-gradient-to-br from-orange-200 to-amber-200 dark:from-orange-300 dark:to-amber-300 rounded-lg group-hover:from-orange-300 group-hover:to-amber-300 dark:group-hover:from-orange-400 dark:group-hover:to-amber-400 transition-all duration-300"
                  whileHover={{ rotate: 360 }}
                  transition={{ duration: 0.5 }}
                >
                  <ClipboardList className="h-6 w-6 text-orange-700 dark:text-orange-800" />
                </motion.div>
                <Badge className="bg-orange-600 dark:bg-orange-700 text-white hover:bg-orange-700 dark:hover:bg-orange-800">
                  Descriptions
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              ...
              <CardDescription className="mb-4 text-gray-700 dark:text-gray-300">
                Create and manage reusable work descriptions for faster
                invoicing
              </CardDescription>
              <motion.div
                whileHover={{ scale: 1.02 }}
              ...
              >
                <Button
                  className="w-full bg-orange-600 hover:bg-orange-700 dark:bg-orange-700 dark:hover:bg-orange-800 text-white border-0 font-medium"
                  onClick={() => router.push("/work-descriptions")}
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
          <AnimatedCard delay={1.0} className="gradient-card motion-safe">
            ...
            <CardContent>
              ...
              >
                $
                {stats.totalRevenueAll.toLocaleString("en-US", {
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 lg:space-x-6 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="space-y-1 lg:space-y-2 mt-1">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 text-sm text-slate-600 dark:text-slate-200 space-y-1 sm:space-y-0">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 text-sm text-slate-500 dark:text-slate-300 space-y-1 sm:space-y-0">
                              ...
                              {invoice.dueDate && (
                                <div className="flex items-center">
                                  <Calendar className="h-3 w-3 mr-1 text-red-600 dark:text-red-400 flex-shrink-0" />
                                  Due:{" "}
                                  {new Date(
                                    invoice.dueDate
                                  ).toLocaleDateString()}
                      ...
                      <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row items-start sm:items-center lg:items-end xl:items-center space-y-2 sm:space-y-0 sm:space-x-4 lg:space-x-0 lg:space-y-2 xl:space-y-0 xl:space-x-4">
                        <div className="text-left sm:text-right">
                          <p className="font-bold text-lg lg:text-xl gradient-text">
                            $
                        ...
                        <div className="flex items-center space-x-1 lg:space-x-2">
                          ...
                          >
                            <Button
                              ...
                              className="cursor-pointer p-1 hover:bg-transparent"
                            >
                              <Trash2 className="h-4 w-4 text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300 transition-colors duration-200" />
                            </Button>
                          </motion.div>
                        </div>
                      </div>
