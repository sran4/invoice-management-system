...
function CreateInvoicePageContent() {
  ...
  const [showWorkDescriptionDropdown, setShowWorkDescriptionDropdown] =
    useState(false);
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      const template = localStorage.getItem("selectedTemplate");
      console.log(
        "Storage change detected, template:",
        template,
        "current:",
        selectedTemplate
      );
      if (template && template !== selectedTemplate) {
        ...
        toast.success(
          `Template changed to ${
            templateConfigs[template as keyof typeof templateConfigs]?.name ||
            template
          }`,
          {
  ...
  useEffect(() => {
    if (
      showWorkDescriptionDropdown &&
      searchDropdownRef.current &&
      containerRef.current
    ) {
      const dropdownElement = searchDropdownRef.current;
      ...
      setTimeout(() => {
        ...
        // Check if dropdown is visible in container
  ...
  const filteredWorkDescriptions = workDescriptions.filter(
    (workDescription) =>
      workDescription.title
        .toLowerCase()
        .includes(workDescriptionSearch.toLowerCase()) ||
  ...
  const validateAllFields = () => {
    ...
    errors.invoiceNumber = validateField(
      "invoiceNumber",
      formData.invoiceNumber
    );
  ...
  return (
    <div className="pt-20 p-6 min-h-screen">
      <div
        ref={containerRef}
        className="max-w-6xl mx-auto max-h-[calc(100vh-8rem)] overflow-y-auto pb-8 scroll-smooth scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800"
      >
        {/* Header */}
        <div className="mb-8">
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              ...
              <p className="text-white text-lg">
                Create a new invoice using the{" "}
                <span
                  className={`font-semibold bg-gradient-to-r ${currentTemplate.gradient} bg-clip-text text-transparent`}
                >
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <GradientTooltip content="Due date is auto-calculated from issue date. You can change the default number of days in Settings.">
                        <div className="relative">
                          <Calendar className="absolute left-3 top-3 h-4 w-4 text-blue-400" />
                    ...
                    <div className="p-4 bg-slate-800 rounded-lg border border-slate-700">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          <div
                            className={`p-2 rounded-lg bg-gradient-to-r ${currentTemplate.gradient} bg-opacity-20 border border-slate-600`}
                          >
                            <currentTemplate.icon
                              className={`w-5 h-5 ${currentTemplate.iconColor}`}
                            />
                          </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-slate-300">
                        Search and add pre-saved work descriptions to your
                        invoice
                      </CardDescription>
                    </div>
                    <Button
                      ...
                      onClick={() =>
                        router.push(
                          "/work-descriptions/new?returnTo=invoices/create"
                        )
                      }
                  ...
                  {workDescriptions.length > 0 ? (
                    <div className="relative" ref={searchDropdownRef}>
                      ...
                      {showWorkDescriptionDropdown && workDescriptionSearch && (
                        <div className="mt-2 bg-slate-900 border border-slate-600 rounded-lg shadow-xl max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-500 scrollbar-track-slate-800 hover:scrollbar-thumb-slate-400 animate-in slide-in-from-top-2 duration-200 dropdown-scroll">
                          {filteredWorkDescriptions.length > 0 ? (
                            <div className="py-1">
                              {filteredWorkDescriptions.map(
                                (workDescription) => (
                                  <div
                                    key={workDescription._id}
                                    onClick={() =>
                                      addWorkDescription(workDescription)
                                    }
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        {workDescription.rate > 0 && (
                                          <div className="flex items-center mt-2 text-sm text-green-400">
                                            ...
                                            <span>
                                              ${workDescription.rate.toFixed(2)}
                                              /hour
                                            </span>
                                          </div>
                                        )}
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                        Create your first work description to quickly add items
                        to invoices
                      </p>
                      <Button
                        type="button"
                          ...
                          <div className="col-span-5 space-y-2">
                            ...
                            {item.description.length > 180 && (
                              <p className="text-xs text-amber-500">
                                ⚠️ Approaching character limit for clean
                                printing
                              </p>
                            )}
                          </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3 pt-4">
                    ...
                    <Button
                      ...
                      } text-white border-0 hover:opacity-90 cursor-pointer ${
