...
export const generateInvoicePDF = async (
  ...
  const templateConfig =
    templateConfigs[invoice.template as keyof typeof templateConfigs] ||
    templateConfigs["modern-blue"];

  // Debug log to verify template selection
  console.log(
    `Generating PDF with template: ${invoice.template} (${templateConfig.name})`
  );
  ...
  doc.setFillColor(
    templateConfig.headerGradient[0],
    templateConfig.headerGradient[1],
    templateConfig.headerGradient[2]
  );
  ...
  invoice.items.forEach((item, index) => {
    ...
    doc.text(`$${item.amount.toFixed(2)}`, pageWidth - 25, yPosition, {
      align: "right",
    });
  ...
  doc.text(`$${invoice.subtotal.toFixed(2)}`, pageWidth - 25, yPosition, {
    align: "right",
  });
  yPosition += 8;
  ...
  if (invoice.tax > 0) {
    ...
    doc.text(`$${invoice.tax.toFixed(2)}`, pageWidth - 25, yPosition, {
      align: "right",
    });
    yPosition += 8;
  }
  ...
  if (invoice.discount > 0) {
    ...
    doc.text(`-$${invoice.discount.toFixed(2)}`, pageWidth - 25, yPosition, {
      align: "right",
    });
    yPosition += 8;
  }
  ...
  doc.text(`$${invoice.total.toFixed(2)}`, pageWidth - 25, yPosition, {
    align: "right",
  });
  ...
  if (currentPage === totalPages) {
    ...
    doc.text(`Generated: ${timestamp}`, pageWidth - 20, footerY, {
      align: "right",
    });
  }
